...
class ImageGraph:
    ...
    def create_adjacency_matrix(self):
        ...
        """
        v_length = len(self.vertices)
        matrix = []
        for _ in range(v_length):
            row = []
            for _ in range(v_length):
                row.append(0)
            matrix.append(row)
...
def create_graph(data):
    ...
    size = int(lines[0])
    v_length = int(lines[1])
    # create the ImageGraph
    graph = ImageGraph(size)
    # create vertices - vertex info has the format "x,y,color"
    for i in range(v_length):
        x, y, color = lines[i + 2].split(",")
        graph.vertices.append(ColoredVertex(i,int(x), int(y), color))
    ...
    e_length = int(lines[v_length + 2])
    edge_start = v_length + 3
    for i in range(edge_start, edge_start + e_length):
        connect = lines[i].split(',')
        graph.vertices[int(connect[0])].add_edge(int(connect[1]))
        graph.vertices[int(connect[1])].add_edge(int(connect[0]))

    # read search starting position and color
    last = tuple(lines[3 + v_length + e_length].split(","))
    start_index = int(last[0])
    color = last[1]
